...
- **fal.ai** — images + video. Models in use: **Nano Banana Pro** (character portraits + composing characters into one scene shot via `/edit`), **Kling v3 standard i2v** (used for BOTH dialogue SCENES — two characters talking to each other — AND the memoir NARRATION — the lone lead performing to camera — all in our cloned voices), **Kling create-voice** (clone an ElevenLabs sample → reusable voice_id). Every character — including people the article doesn't name — gets a real locked Nano Banana portrait (no more FLUX silhouettes; they broke consistency in the scene pipeline). (Seedance is no longer used — narration is now a real Kling performance, not silent motion.)
- **ElevenLabs** (v3) — voices, all cloned into Kling: the dialogue characters AND the lead who narrates (the narration is spoken in the lead's OWN cloned voice, not a separate narrator).
...
| 5. Voices | `voice_maker.py` | ElevenLabs + Kling create-voice (clone) | voice_ids on every speaking char + the narrating lead | by characters |
| 6. Scene clips | `scene_clips.py` | Nano Banana Pro (compose chars in one shot) + Kling v3 (dialogue scenes AND memoir narration, in cloned voices) | `clip_*.mp4` | ~$0.154/sec |
...
- `scene_writer.py` writes 5–7 SCENES (each dialogue scene ≤2 speaking characters; narration = the lead, first person, to camera). `voice_maker.py` clones each speaking character's ElevenLabs voice into a Kling `voice_id` (create-voice), including the narrating lead. `scene_clips.py` composes the characters into one shot (`nano-banana-pro/edit`) and animates it as a Kling v3 clip in the cloned voices — dialogue scenes (2 people, no camera) and narration (the lone lead performing to camera). `assemble.py` joins the scenes. (Seedance retired.)
