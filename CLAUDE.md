...
- **fal.ai** — images + video. Models in use: **Nano Banana Pro** (character portraits + composing characters into one scene shot via `/edit`), **Seedance 1.5 pro i2v** (animates each scene — two people acting + talking in ONE shot, native lip-sync — and the memoir narration). Every character gets a real locked Nano Banana portrait (no FLUX silhouettes). Seedance replaced **Veo 3.1**: Veo's likeness filter refused our AI-invented faces on ~half of all clips (a Google policy that fires on fal, the Gemini API AND Vertex — not promptable-around) and cost more; Seedance holds the same faces, isn't blocked, and is ~$0.052/s. A model shoot-out (Kling 2.6 pro / Seedance 1.5 pro / Wan 2.5) picked Seedance on cost + face quality.
- **ElevenLabs Speech-to-Speech** (`eleven_english_sts_v2`) — Seedance invents its own voice per clip, so for the recurring NARRATOR we SWAP it for the lead's locked ElevenLabs voice after generation; sts keeps the timing so the lip-sync still matches, and the lead sounds identical across every video. Dialogue clips currently keep Seedance's native voices (re-voicing both speakers in a 2-person clip needs per-speaker audio splitting — a later upgrade). (Veo / Kling retired.)
...
| 6. Scene clips | `scene_clips.py` | Nano Banana Pro (compose chars in one shot) + Seedance 1.5 pro (one clip PER SCENE, native lip-sync) + ElevenLabs Speech-to-Speech (swap narrator to our voice) | `clip_*.mp4` | ~$0.052/sec Seedance |
...
- ~$3–5 per finished ~75s video (Seedance is the bulk). Optional `output/music.mp3` adds background music. No captions.
...
- Microdramas are mostly **talking close-ups + music** — that is the right format, not a limitation. (We use NO dialogue subtitles. Short **location cards** — a place name burned briefly over a new scene, Law & Order style — ARE allowed and on by default; they're orientation, not captions. Toggle `LOCATION_CARDS` in `assemble.py`.)
...
- **Voice consistency (Seedance + swap):** Seedance invents a NEW voice every clip and can't be pinned. The recurring NARRATOR is a one-speaker clip, so we run its audio through **ElevenLabs Speech-to-Speech** (`revoice`) into the lead's fixed `voice_id` — sts keeps the timing, so the lip-sync still matches and the lead sounds identical across every video. (Validated on Veo: swapping the voice did NOT break the mouths; the mechanism is model-agnostic.) Dialogue clips currently keep Seedance's native voices — re-voicing BOTH speakers in a 2-person clip would need per-speaker audio splitting (a later upgrade).
...
- **Scene transitions — smooth, like a real short film (do NOT dip-to-black everywhere).** Research (pro editors + AI-microdrama makers) is blunt: ~9,990 of 10,000 film transitions are HARD CUTS, and a fade-to-black means "chapter over" — firing it at every scene reads as five chapters in forty seconds, or devalues to dead frames that stall the pace (on a feed, scroll away). So the grammar is:
  1. **Hard cut by default.** Every scene change is a straight cut.
  2. **Dip to black ONLY on a real time jump.** `scene_writer.py` sets `time_jump` per scene; `assemble.py` fades only those. (The old "dip whenever place/people change" was wrong — reverted.)
  3. **Ambience J-cut carries the change (the core fix).** `build_ambient_bed` starts the NEXT room's tone `PRELAP`≈0.6s BEFORE the picture cuts, crossfaded — the ear arrives in the new room just before the eye, so a hard cut lands as intended, not as a jolt. Free, and it never touches lip-sync (only the bed moves).
  4. **Detail insert opens each new location.** `scene_clips.py` composes a face-free object (the scene's `detail` field — a gavel, a nameplate) with Nano and animates a short SILENT Seedance clip; the editor holds it ~1.2s (`INSERT_MAX`). Modern replacement for the establishing wide; no faces so nothing drifts. ~$0.28/location ($0.15 Nano + ~$0.13 silent Seedance: 5s × $0.026/s no-audio).
  5. **Continuous ducked music** + **location cards** glue and label the change. Music (`make_music`, one ElevenLabs loop) runs unbroken and is sidechain-DUCKED under the voices. Cards are burned via a PIL PNG overlay because this ffmpeg build has NO `drawtext` (no libfreetype) — check `overlay_card`.
  6. **Script-side orientation:** narration bridges NAME the new place/person before we cut there; new characters are **pre-named before their face appears** (`scene_writer.py`); "get in late, leave early"; narration says what the picture CAN'T (never describes the shot). `analyze.py` rejects **confusable invented names** (Rajan/Rowan) via edit-distance.
- **NOT doing (researched, rejected):** morph/keyframe transitions (vendors admit they break across locations), whip pans (motion-sick in 9:16), wipes/glitches (stock-template look).
- **The still-image establishing beat was REMOVED** (a photo on a slow zoom next to real motion reads as the video freezing). Detail inserts are real footage instead.

## Current status
- **SEEDANCE SCENE pipeline:** `run.py` takes a link → a short-film video where two characters act and talk to each other in one shot, in OUR voices. ~$3–5 per ~75s video. Seedance 1.5 pro replaced Veo 3.1 (Veo's likeness filter blocked our AI faces on ~half of clips and cost $0.10–0.15/s); Seedance holds the same faces, does native lip-sync, isn't blocked, and is $0.052/s. Chosen over Kling 2.6 pro and Wan 2.5 in a live shoot-out on cost + face quality.
- `scene_writer.py` writes 5–7 SCENES (unchanged: ≤2 speakers + `onscreen` cast; narration = the lead, first person, to camera). `voice_maker.py` assigns each character a fixed ElevenLabs `voice_id` (free). `scene_clips.py` composes the on-screen cast into one shot (`nano-banana-pro/edit`), then renders ONE **Seedance 1.5 pro** clip for the whole scene (native lip-sync). The narration clip (protagonist to camera) is **re-voiced** into the lead's ElevenLabs voice (`revoice`, Speech-to-Speech); dialogue keeps Seedance's native voices. `assemble.py` joins the scenes.
- Cost tiers: Seedance 1.5 pro i2v **$0.052/s** with audio (**$0.026/s** silent, 720p); ElevenLabs Speech-to-Speech **$0.002/s**; Nano Banana compose **$0.15/image**.
- **Upgrade path (later):** re-voice BOTH speakers in a dialogue clip (needs per-speaker audio splitting / diarisation before the swap) so every voice is ours, not just the narrator's. Also stronger cross-scene identity via a reference-to-video model.
