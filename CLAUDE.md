...
- **Reuse the scene image across scenes:** don't compose a new image every scene. Key it by (cast + location) and REUSE it while the same people stay in the same place; only compose a new one when a character enters/leaves or the location changes. Keeps the room visually continuous scene-to-scene AND saves a $0.15 render each time (`scene_img_cache` in `scene_clips.py`).
